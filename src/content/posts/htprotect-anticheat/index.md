...
| Redroid（Docker 容器云机）                               | `ro.kernel.redroid.{fps,height}`、`ro.boot.redroid_fps`                                                                                                                                                               |
...
这些判据覆盖特征二进制、`.so`、`/dev` 设备节点、`/sys/module` 内核模块、`/data/data` 应用目录、`init.*.rc` 与 `ro.*`/`init.svc.*` 系统属性。看得出来，易盾是照着每个平台跑起来会留下什么痕迹逐个抠的，覆盖面比我一开始预想的全。
