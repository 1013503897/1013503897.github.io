---
title: 浅析 Google Play 自动保护（PairIP）——代码虚拟化、TEE 密钥与运行时反篡改
summary: >-
  Play 自动保护三道门：TEE 密钥门、反篡改 SIGSEGV 自毁、hook 即自毁的完整性门；改用「调用」而非 hook 脱出 28/28 段 VM
  明文。
created: '2026-08-17'
tags:
  - 移动安全
...
最近在分析 Google Play 上的 Unity app，碰到过好几次这样的情况：JADX 拉开关键方法只剩一句 `VMRunner.invoke("...")`，逻辑全被掏空；想扔模拟器上跑起来看看，又几秒就退。这两件事其实是同一层加固的两面。这层加固是 Google Play 的 [Automatic Protection](https://support.google.com/googleplay/android-developer/answer/10183279)（SDK 代号 PairIP），上架时由 Play 侧自动套上，把「代码虚拟化 + TEE 硬件密钥 + 运行时反篡改」打成一个整体。下文只拆这层保护本身、不碰样本 app 的业务逻辑，结论对任意 PairIP 目标通用。仅供技术交流。

本文要回答的问题：

- 被保护的代码变成了什么形态、存在哪、以什么方式加密；
- 解密密钥在哪一层、谁在什么条件下发放，哪些环境注定拿不到；
- 运行时反篡改到底检测什么、怎么处置，为什么「把自杀函数都 hook 掉」这条路是错的；
- 在满足条件的机器上，能不能稳定把虚拟化字节码脱成明文，用什么姿势不踩自毁。

最终目的：在一台真机上让它跑到交互态（等于同时过了密钥门和反篡改门），并把全部加密 VM blob 解密落盘成明文（用 `\x00IAP` 头做客观判据）。能跑、能脱，才算真正解出，而不止于在 JADX 里看清结构。

复现环境固定成一套三绿真机，后面所有现象都在它上面观察：

| 层 | 用什么 |
|---|---|
| 静态 | `unzip -l base.apk` + JADX |
| 设备 | Pixel 6（oriole，Android 16，`6.1.145-android14`），三绿真机 |
| root / Zygisk | 内核态 root + Zygisk-Next（`su 0` 全能力、有模块控制面） |
| Integrity | PlayIntegrityFix，让设备过 Play Integrity，服务端才会签发 TEE 密钥 |
| 分发 | Play 正版安装（带 `gpdeku` split，密钥载体） |
| 动态 | 一个用户态反检测 frida（在目标进程里隐藏 frida 自身的 maps / 端口 / 线程），client 与设备 daemon 严格同版本，否则不握手 |

---

## 一、先认出它，再看被保护的逻辑藏在哪

先交代下来历：`com.pairip.*` 这套壳早在 2023 年就被独立研究者记录在案（那会儿还主要把它当成一个运行时签名校验壳，见文末 Narendra Dwivedi），后来才被 Google 并进 Play 的 Automatic Protection、在上架时大规模自动下发，长成今天「代码虚拟化 + TEE 密钥 + 运行时反篡改」三件套的样子。所以它不是某个第三方加固商的产品，而是 Play 发布链路自带的一层。

PairIP 的静态特征很明确：native 层有 `lib/arm64/libpairipcore.so`（VM 解释器 + 反篡改都在这），Java 层有 `com.pairip.*` 包，assets 顶层躺着一批随机名、没扩展名的文件，就是加密后的 VM blob。Play 版还会比 sideload 多两个 split（`gpdeku` / `gpdeku.config.arm64_v8a`），这俩是密钥下发的载体，sideload 装出来没有，这也是后面非三绿环境跑不起来的原因之一。

进程起来后反射枚举 `com.pairip.*`（只读，不触发反篡改，原因见第四节），能拿到二十多个类，把加固的骨架勾出来：
...
启动次序是理解「无密钥直接退出」的关键。`InitContextProvider` 在 Application 之前就跑，配合 AppComponentFactory 做组件替换：拿到密钥、校验通过，才把真实组件换上、由 `StartupLauncher.launch()` 驱动 VM；任一环不过，真实组件就被替换成占位实现，然后进程自己退出。整条启动门大致是：

![PairIP 启动门：InitContextProvider 先于 Application 跑，StartupLauncher 经 LicenseClient 在线授权 + 反篡改/环境检查；密钥就绪且检查通过才装入真实组件、走到 VMRunner.invoke → executeVM，否则组件换占位实现并 System.exit(0)/自毁](./assets/images-fig-startup-gate.png)
...
![native executeVM 是 JNI 薄壳：取出 byte[] 字节码后转交 VM 解释器](./assets/images-fig-executevm-shim.png)
...
```bash
touch /data/adb/modules/<inject-module>/disable   # 下次开机不再加载这个可见注入
reboot                                             # PIF 保留，三绿不丢
```
...
![脱壳流程：列出 assets/ 顶层 blob 名 → 反检测 frida attach（maps 检测不命中）→ 逐个 blob 反射调用 VMRunner.getVmByteCode（调用而非 hook，带/不带 assets/ 前缀各试一遍）→ 头 4 字节 == \x00IAP 即明文落盘，否则换前缀/跳过](./assets/images-fig-dump-flow.png)
...
- 官方：[Google Play — Automatic protection / Protected with Play](https://support.google.com/googleplay/android-developer/answer/10183279)
