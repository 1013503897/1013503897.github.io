...
> **目标 App**：`com.globe.gcash.android`（GCash）6.00.2 build 1213（真机动态另测 6.01.0 build 1216）
> **样本**：`gcash_1213_base.apk`（jadx 索引约 8.2 万类）
> **对象层**：自研 DynamicSecurity（`RequestEncryption` / `GAESCipher` / `GRSACipher`），不是 Alipay mPaaS 标准 RPC
...
1. 抓包面先分诊：系统代理被探测时，先判定是证书 pin 还是代理设置探测，再决定入口。
2. 同名 OTP / 标准 RPC 类名不可信：从 Activity → Repository → Retrofit `@POST` 逐层坐实。
3. anti-decompile 时读 smali 调用序：关键逻辑往往只是噪声 `goto`，调用图仍清晰。
4. 字段级加密先找 encParams + sec.enc 路径清单，它们是「谁被加密」的权威索引。
5. 序列化字节等价是验签成败的真变量：gson `=` 转义 / 紧凑分隔 / null 省略 / ART 字母序字段顺序四条缺一不可；用离线 oracle（自造服务器对解回）把「签对没」与「服务器收不收」解耦。
6. 误导性信号用对照表排除：`os` 字段 vs getColorInfo 白盒；`code:0` + 下一跳 vs `key:null` 文案；`rdsData` 空串 vs 非空；umid vs `needUmid=false`。删减/翻转单一变量做 A/B 定位。
7. 选对 hook 层：native 有 CRC 自校时，往上找 dex/ART 层的 choke point（`generateSignedBody`），既拿全明文又不触发完整性检查。
8. 可复现性分级：把每个标识按「可造/可铸/硬缺口」归类（apdid=铸造端点绕过、utdid=算法可铸、umid=无关），硬缺口才需上真机。
